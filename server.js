    ...
    if (!isQuestion(msg)) {
      if (isGreating(msg)) {
        ...
        setTimeout(() => {
          ...
          io.emit(
            "message", botAnswer + "Hi " + userName[0] + "! Ask me the weather or time."
          );
        }, 800);
      } else {
        io.emit("message", msg);
      ...
      io.emit("message", msg);
      setTimeout(() => {
        io.emit("message", botAnswer + "It is "+ new Date().toLocaleTimeString() + " in your timezone.");
      }, 800);
    } else if (askingWeather(msg)) {
      io.emit("message", msg);
      getWeather(function (weather) {
        io.emit("message", botAnswer + " It is going to be " + weather.toLowerCase() + " right now.");
      }, 800);
    }
  });
});
